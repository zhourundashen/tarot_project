    ...
    async chat(messages) {
        ...
        if (CONFIG.useBackend) {
            try {
                ...
                if (!response.ok) {
                    throw new Error(`后端API失败: ${response.status}`);
                }
                ...
                console.warn('后端调用失败，尝试直接调用API:', error);
...
- 语气温暖友善`;
    ...
    buildSummaryPrompt(userQuestion, spread, cards, conversationHistory) {
        const cardList = cards.map((item, i) => {
            const pos = spread.positions[i];
            return `${i + 1}. 【${pos.name}】${item.card.name}（${item.reversed ? '逆位' : '正位'}）`;
        }).join('\n');

        const historyText = conversationHistory && conversationHistory.length > 0 
            ? conversationHistory.map(h => `用户：${h.question}\nAI：${h.answer}`).join('\n\n')
            : '无追问';

        return `请为本次占卜生成深度总结。

【占卜背景】
用户问题：${userQuestion || '无具体问题'}
使用牌阵：${spread.name}
...
【总结要求】
作为专业塔罗师，请生成一份有深度的占卜总结报告，帮助用户全面理解本次占卜的启示。

【总结格式】

═══════════════════════════════════
【核心信息】（2-3句话）
提炼本次占卜的核心信息，融合所有牌的启示，给出整体判断。
...
═══════════════════════════════════
【问题解答】（3-4句话）
${userQuestion ? `针对用户的问题"${userQuestion}"，结合牌面给出明确的答案和建议。` : '针对用户当前的状况，给出综合性的指引和建议。'}
...
═══════════════════════════════════
【心理层面建议】（2-3句话）
从心理学角度，建议用户以怎样的心态面对当前情况，如何调整心理状态。

═══════════════════════════════════
【灵性指引】（2-3句话）
从灵性和能量层面，给予用户更深层的指引和启示，帮助用户连接内在智慧。

═══════════════════════════════════
【关键时间节点】（如果适用）
指出可能的重要时间点或转折期，以及需要注意的时机。

═══════════════════════════════════
【总结关键词】（5-7个）
用关键词概括本次占卜的核心主题，用顿号分隔。

═══════════════════════════════════
【温暖寄语】（3-4句话）
以温暖、鼓励的语气，给予用户力量和希望，让用户感受到被理解和支持。
...
- 让用户感受到被理解和被支持`;
    ...
    async getInitialReading(userQuestion, spread, cards, onProgress) {
        const systemPrompt = {
            role: 'system',
...
- 快速响应，抓住重点`
        ...
        const userPrompt = {
            role: 'user',
            content: await this.buildInitialPrompt(userQuestion, spread, cards)
        };
        
        const fullReading = await this.chat([systemPrompt, userPrompt]);
        
        if (onProgress && cards.length >= 7) {
            return { fullReading, progressive: true };
        }
...
- 擅长总结和提炼关键信息`
    ...
    async chatWithContext(message, userQuestion, spread, cards, chatHistory) {
        ...
        const cardsInfo = cards && cards.length > 0 
            ? cards.map((item, i) => `${i + 1}. ${item.card.name}${item.reversed ? '（逆位）' : ''}`).join('\n')
            : '尚未抽牌';
